...
protected:
	...
	/// * 限制周期内最大访问次数
	/// * 仅当 [limit_type] = [MyHitLimitType_e::Refuse]，且 [limitSize] > 0 时有效
	...
	/// 记录限制的值和限制时长
	std::map<T, time_t> hitLimit;
	/// 记录一段时间内，某值访问的次数
	std::map<T, int> hitNum;
	
	/// hit的管理周期；单位：s秒；
	time_t hitInterval = -1;
	std::shared_ptr<mimicry::MyTimer_c> timer_task = nullptr;
	// 返回是否清理 [hitNum] 
	std::function<bool(MyHitLimit_c<T>* in_limit)> onHitIntervalControl = nullptr;
...
public:
	MyHitLimit_c(MyHitLimitType_e in_type, 
		int in_limitSize = -1, 
	...
	/// *	当 [in_time] < 0，则不限制时长
	/// *	当 [in_time] = 0 时，使用默认的 [limitAge] 和 当前时间 计算截止时间点
	/// *	当 [in_time] > 0，则只在指定时间点[in_time]内有效
	void add_hitLimit(T in_ip, time_t in_time = 0) {
		if (0 == in_time) {
			this->hitLimit[in_ip] = std::time(nullptr) + limitAge;
		}
		else {
			this->hitLimit[in_ip] = in_time;
		}
	}
	...
	void remove_hitLimit(T in_ip) {
		auto it = this->hitLimit.find(in_ip);
		if (it != this->hitLimit.end()) {
			this->hitLimit.erase(it);
		}
	}
	...
	int add_hit(T in_ip) {
		int renum = 0;
		auto it = this->hitNum.find(in_ip);
		if (it != this->hitNum.end()) {
			renum = (++(it->second));
		}
		else {
			this->hitNum.insert(std::make_pair(in_ip, 1));
			renum = 1;
		}
		return renum;
	}
	...
	bool add_hit(T in_ip, bool autoLimit) {
		...
		if (true == autoLimit) {
			...
			if (MyHitLimitType_e::Refuse == limit_type
				&& limitSize > 0
				&& num >= limitSize) {
				return false;
			}
	...
	std::map<T, int>& get_hit() {
		return this->hitNum;
	}

	int get_hit(T in_ip) {
		return this->hitNum[in_ip];
	}

	/// 设置IpHit的周期管理事件执行函数
	/// *	如果 [in_ipHitInterval] 小于等于 0，则 取消管理函数
	void set_onHitIntervalControl(time_t in_ipHitInterval, 
		const std::function<bool(MyHitLimit_c<T>* in_limit)>& in_onIpHitIntervalControl) {
		// 取消之前的定时器
	...
	/// <param name="in_time">待检查时间点(单位：s秒)；如果 小于等于 0, 则取调用时的时间点进行判断</param>
	/// <returns>是否放行访问</returns>
	bool isAllow(T in_ip, time_t in_time = -1) {
		...
		case MyHitLimitType_e::Allow:
			...
			if (in_time <= 0) {
				...
				// 当前时间小于等于允许ip的时长时放行
				return (now_time <= it->second);
			}
			else {
				return (in_time <= it->second);
			}
			break;
	...
	static bool defRefuseOnHitIntervalControl(MyHitLimit_c<T>* in_limit) {
		...
		for (auto it = hitNum.begin(); it != hitNum.end(); ++it) {
			if (it->second >= in_limit->limitSize) {
				// 如果访问超速，禁止访问
				MyLog_c::getInstance()->cfinfo(
					fmt::format("Refuse IP {} seconds: {}",
...
public:
	...
	MyHitLimit_c<T>& newItem(MyHitLimitType_e in_type, int in_limitSize = -1, time_t in_limitAge = 0) {
