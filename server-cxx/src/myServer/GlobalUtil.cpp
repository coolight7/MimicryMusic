...
bool mimicry::has_string(const std::string& in_str, const std::initializer_list<const char*>& in_strs)
{
...
bool mimicry::has_char(const std::string& in_str, const std::initializer_list<char>& in_chars)
{
...
std::string& mimicry::escape(std::string& in_str, const std::initializer_list<char>& in_chars)
{
	if (in_str.empty())
		return in_str;
	const char* endch = in_chars.end(), *ch;
...
std::string mimicry::escape(const std::string& in_str, const std::initializer_list<char>& integers)
{
...
bool mimicry::isFormat_limitChar(const std::string& in_str,
	const std::initializer_list<char>& in_chars,
	size_t in_minLen,
	size_t in_maxLen
) {
...
bool mimicry::isFormat_ipv4(const std::string& in_ipv4) {
	regex reg_passwd{ "(25[0-5]|2[0-4]\\d|[0-1]?\\d?\\d)(\\.(25[0-5]|2[0-4]\\d|[0-1]?\\d?\\d)){3}"};
...
bool mimicry::isExist(const std::multimap<std::string, std::string>& in_map,
...
/*
bool mimicry::get_str_encoding(const std::string& in_str, size_t in_use_len, std::string& re_encoding) {
	in_use_len = (in_use_len <= in_str.size()) ? in_use_len : in_str.size();
	...
	if (status == U_ZERO_ERROR) {
		...
		if (status == U_ZERO_ERROR) {
			...
			if (status == U_ZERO_ERROR && matchCount > 0) {
				re_encoding = ucsdet_getName(csm[0], &status); //分配了内存， 需要释放
				if (status == U_ZERO_ERROR){
					rebool = true;
				}
			}
...
int mimicry::to_encoding(const std::string& in_str,
	...
	ucnv_convert(to_encoding.c_str(), from_encoding.c_str(), re_str_p, new_len, in_str.c_str(), in_str_len, &error);
	re_str = re_str_p;
	delete[]re_str_p;
	return error;
}
bool mimicry::to_encoding_utf8(const std::string& in_str,
	...
	if (mimicry::get_str_encoding(in_str, in_str_len, from_encoding)) {
		if (mimicry::to_encoding(in_str, in_str_len, re_str, from_encoding, "UTF-8") == 0) {
			return true;
		}
	}
	return false;
...
*/
std::wstring mimicry::to_wstring(const std::string str)
{// string转wstring
...
std::string mimicry::to_string(const std::wstring str)
{// wstring转string
