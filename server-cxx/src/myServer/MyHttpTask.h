...
namespace mimicry {
	...
	public:
		static const char * Form_data,
			...
			* Text,
			* Binary;
		static constexpr int Form_data_int = 1,
			X_www_form_urlencoded_int = 2,
			Json_int = 4,
			Xml_int = 8,
			Text_int = 16,
			Binary_int = 32;
	...
	std::multimap<std::string, mimicry::HTTP_formData_s> read_form_data(const std::string& in_bound, const std::string& in_data);
...
public:
	...
	void out_entity(const T& in_entity) noexcept {
		try {
			...
			try {
				auto entity = MyRespBaseEntity_c<int>::error(mimicry::ResponeState_e::Exception, "");
