...
public:
	T& data;
	int type;
	MyCellItem_s(T& in_data, int in_type) :data(in_data), type(in_type) {}
};
...
protected:
	...
	MySqlTask_c(WFMySQLTask* in_task, const MySqlCallback_t& in_callback) noexcept
		:wfTask(in_task),
...
public:
	...
	bool is_success() {
		return (this->wfTask->get_state() == WFT_STATE_SUCCESS
	...
	std::vector<std::vector<protocol::MySQLCell>> get_firstSql_result() {
		...
		if (this->is_success()) {	//sql成功
			...
			if (cursor.get_cursor_status() == MYSQL_STATUS_GET_RESULT) {	//有结果返回
	...
	static MySqlTask_c* getMyTask(WFMySQLTask* in_task, const MySqlCallback_t& in_callback) {
	...
	/*判断in_arr中元素类型是否正确
	*/
	static bool is_type(const std::vector<protocol::MySQLCell>& in_arr, const std::initializer_list<int> in_types) {
		...
		for (; t_it != t_endit; ++c_it, ++t_it) {
			if (*t_it & MyCellType_e::Null) {
				if(c_it->is_null())
					continue;
				else if (*t_it == MyCellType_e::Null)
					return false;
			}

			if (*t_it & MyCellType_e::Int) {
				if (c_it->is_int() == false) {
					return false;
				}
			}
			else if (*t_it & MyCellType_e::Double) {
				if (c_it->is_double() == false && c_it->is_float() == false) {
					return false;
				}
			}
			else if (*t_it & MyCellType_e::Ulonglong) {
				if (c_it->is_ulonglong() == false) {
					return false;
				}
			} 
			else if (*t_it & MyCellType_e::Str) {
				if (c_it->is_string() == false) {
					return false;
				}
			}
			else if (*t_it & MyCellType_e::Datetime) {
				if (c_it->is_datetime() == false) {
					return false;
				}
			}
			else if (*t_it & MyCellType_e::Time) {
	...
	template<typename T>
	static bool to_type(std::vector<protocol::MySQLCell>::const_iterator in_it, MyCellItem_s<T>&& in_item) {
		int type = in_item.type;
		if constexpr (std::is_same_v<T, unsigned long long> 
			|| std::is_same_v<T, long long> 
			|| std::is_same_v<T, unsigned long>
			|| std::is_same_v<T, long>) {
			if (type & MyCellType_e::Ulonglong && in_it->is_ulonglong()) {
		...
		if constexpr (std::is_same_v<T, int> || std::is_same_v<T, unsigned int>
			...
			|| std::is_same_v<T, unsigned long>
			|| std::is_same_v<T, long>) {
			if (type & MyCellType_e::Int && in_it->is_int()) {
			...
			if (type & MyCellType_e::Str && in_it->is_string()) {
				in_item.data = in_it->as_string();
				return true;
			} 
			else if (type & MyCellType_e::Datetime && in_it->is_datetime()) {
				in_item.data = in_it->as_datetime();
				return true;
			}
			else if (type & MyCellType_e::Time) {
				if (in_it->is_time()) {
	...
	static bool to_type(std::vector<protocol::MySQLCell>::const_iterator in_it, 
		MyCellItem_s<T>&& in_item, 
		MyCellItem_s<Args>&&... in_args) {
		return to_type(in_it, std::forward<MyCellItem_s<T>>(in_item)) && to_type(in_it + 1, std::forward<MyCellItem_s<Args>>(in_args)...);
	}
	...
	*/
	...
	static bool to_type(std::vector<protocol::MySQLCell>::const_iterator in_begin, 
		std::vector<protocol::MySQLCell>::const_iterator in_end, 
		MyCellItem_s<Args>&&... in_args) {
		if (in_end - in_begin == sizeof...(in_args)) {
			return to_type(in_begin, std::forward<MyCellItem_s<Args>>(in_args)...);
		}
		return false;
	}
