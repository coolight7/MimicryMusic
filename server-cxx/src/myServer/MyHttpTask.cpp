...
std::multimap<std::string, mimicry::HTTP_formData_s> MyHttpRequest_c::get_form_data() {
	...
	string type, body;
	this->find_header("Content-Type", type);
	if (std::regex_search(type, bound_m, std::regex(std::string(mimicry::HttpBodyType_e::Form_data) + "\\s*;\\s*boundary\\s*=\\s*(.+)"))) {
...
{
	WFServerTask<MyHttpRequest_c, MyHttpResponse_c>::set_callback(
		[this](MyHttpTask_c* task) {
			this->run_callback();
		});
}
