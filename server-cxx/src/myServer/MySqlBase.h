...
public:
	...
	MySqlTask_c* get(
		...
		return mytask;
		//return conn.create_query_task(query, std::bind(fun, std::placeholders::_1, in_callback, before_callback));
		/*
	...
	MySqlTask_c* dosql_errlog(const std::string& in_sql,
		...
		if (in_log) {
			auto fun = [in_log, fileName, funName, lineNum](MySqlTask_c* mytask, const MySqlCallback_t& callback) {
			...
			mytask = this->get(
				in_sql,
				std::bind(fun, std::placeholders::_1, callback)
			);
		}
		else {
			mytask = this->get(
...
#define _dosql_errlog(sql, fun, log) \
	dosql_errlog(sql,fun, log, __FILE__, __FUNCTION__, __LINE__)

	MySqlTask_c* dosql_log(const std::string& in_sql,
		const MySqlCallback_t& callback, 
		MyLog_c* in_log, 
		const char* fileName,
		const char* funName,
...
#define _dosql_log(sql, fun, log) \
	dosql_log(sql, fun, log, __FILE__, __FUNCTION__, __LINE__)
	...
	//转义目标字符：斜杠\\ 单引号' 双引号"
	static std::string& escape(std::string& in_str, std::initializer_list<char> integers = { '\\', '\'', '"'}) {
		return mimicry::escape(in_str, integers);
	}
	static std::string escape(const std::string& in_str, std::initializer_list<char> integers = { '\\', '\'', '"' }) {
