...
[](MyHttpTask_c* myTask, MyHttpServer_c* server)
{
	if (server->onRequestStart && server->onRequestStart(myTask) == false) {
		// [server] 的 [onRequestStart] 事件存在，且 [onRequestStart] 返回了 false，阻断继续执行
		return;
	}
...
#define HTML_first			"<html><body style='display:flex;flex-direction: column;justify-content: center;align-items: center;'>"
...
MyHttpServer_c::MyHttpServer_c(const WFServerParams* in_params) noexcept
	...
	log("[MyHttpServer] ") {
	WFServer<MyHttpRequest_c, MyHttpResponse_c>::process = std::bind(serverFun, std::placeholders::_1, this);
	...
	this->router.add("/*",
		MyHttpMethod_e::Get,
		MyHtml_c::defaultHTML_fun(MyHttpState_e::OK)
	);
}

CommSession* MyHttpServer_c::new_session(long long seq, CommConnection* conn) {
	MyHttpTask_c* task = new MyHttpServerTask_c(this, WFServer<MyHttpRequest_c, MyHttpResponse_c>::process);
...
bool MyHttpServer_c::set_config(const string& in_confPath, std::function<bool (nlohmann::json&)>&& in_succCallback) noexcept
{
	...
	if (in_confFile.is_open()) {
		...
		if (conf_json["src"]["html"]["default"].is_null() == false) {
			...
			for (auto& item : defHtml.items()) {
				string key_str = (item.value().get<std::map<std::string, std::string>>()).begin()->first;
				...
				if (false == file_path.empty()) {
					...
					log.cinfo("Set | src - html - default: code:" + key_str + "; file:" + file_path);
