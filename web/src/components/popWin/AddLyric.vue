...
const emits = defineEmits({
    'add':(in_lyric:Lyric_c) => {
        if(in_lyric) {
            return true;
        } else {
            console.log("AddLyric.vue: 事件 'add' 所绑定的函数未接收关键参数");
            return true;
        }
    },
    "cancle":null
...
mValue.lyric.access = Lyric_access_e.Public;
mValue.lyric.copyright = Lyric_copyright_e.Original;

const addList_click = () => {
    if (!regex.value.name.test(mValue.lyric.name)) {
        global_s.tip("歌词名 格式错误");
        return;
    }
    ...
    emits('add', <Lyric_c>mValue.lyric);
}
...
<template>
    <PopWinBase title="创建歌词" :f-btns="btnlist">
