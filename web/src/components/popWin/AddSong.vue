...
const emits = defineEmits({
    'add':(list:Song_c) => {
        if(list) {
            ...
            console.log("AddSong.vue: 事件 'add' 所绑定的函数未接收关键参数");
            return true;
        }
    },
    "cancle":null
...
mValue.song.name = "";
mValue.song.artist = "";
mValue.song.audio = [new MySrc_c<SongAudioSrcInfo_c>(new SongAudioSrcInfo_c(
    MySrcType_e.Bili,
))];
mValue.song.icon = new MySrc_c(new MySrcInfo_c());
...
async function addSong() {
    ...
    mValue.song.sid = Math.floor(Math.random() * 100000000);
    if(mValue.song.icon.src.length > 0) {
        const resrc = await src_s.tryDataURLUpload(mValue.song.icon.src, true);
        if(null != resrc) {
            mValue.song.icon = resrc;
            emits('add', mValue.song);
        }
    } else {
        emits('add', mValue.song);
    }
...
<template>
    <PopWinBase title="添加歌曲" :f-btns="btnlist">
